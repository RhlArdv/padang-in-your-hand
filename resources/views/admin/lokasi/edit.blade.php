...
@section('content')
    <form method="POST" action="{{ route('admin.lokasi.update', $lokasi->id_lokasi) }}" enctype="multipart/form-data" class="max-w-3xl">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-100">
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="nama_tempat" id="nama_tempat" value="{{ old('nama_tempat', $lokasi->nama_tempat) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div x-data="{
                        open: false,
                        search: '',
                        selectedId: '{{ old('id_kategori', $lokasi->id_kategori) }}',
                        selectedLabel: '{{ old('id_kategori') == 'lainnya' ? 'Lainnya (Tulis Baru)' : ($kategoris->firstWhere('id_kategori', old('id_kategori', $lokasi->id_kategori))->nama_kategori ?? 'Pilih Kategori') }}',
                        kategoris: [
                    ...
                    <div>
                        ...
                        <input type="text" name="kontak" id="kontak" value="{{ old('kontak', $lokasi->kontak) }}"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="alamat" id="alamat" rows="2" required
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">{{ old('alamat', $lokasi->alamat) }}</textarea>
                    ...
                    <div>
                        ...
                        <select name="id_kecamatan" id="id_kecamatan" required
                            ...
                            @foreach($kecamatans as $kec)
                                <option value="{{ $kec->id_kecamatan }}" {{ old('id_kecamatan', $lokasi->id_kecamatan) == $kec->id_kecamatan ? 'selected' : '' }}>
                                    {{ $kec->nama_kecamatan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="id_kelurahan" id="id_kelurahan" required
                            ...
                            @foreach($kelurahans as $kel)
                                <option value="{{ $kel->id_kelurahan }}" {{ old('id_kelurahan', $lokasi->id_kelurahan) == $kel->id_kelurahan ? 'selected' : '' }}>
                                    {{ $kel->nama_kelurahan }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude', $lokasi->latitude) }}" required
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">
                    </div>
                    <div>
                        <label for="longitude" class="block text-sm font-semibold text-gray-700 mb-1.5">Longitude <span class="text-red-500">*</span></label>
                        <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude', $lokasi->longitude) }}" required
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">
                    </div>
                </div>
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">{{ old('deskripsi', $lokasi->deskripsi) }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label for="jam_operasional" class="block text-sm font-semibold text-gray-700 mb-1.5">Jam Operasional</label>
                        <input type="text" name="jam_operasional" id="jam_operasional" value="{{ old('jam_operasional', $lokasi->jam_operasional) }}"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">
                    </div>
                    <div>
                        <label for="website" class="block text-sm font-semibold text-gray-700 mb-1.5">Website</label>
                        <input type="url" name="website" id="website" value="{{ old('website', $lokasi->website) }}"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 space-y-5">
                ...
                @if($lokasi->fotos->count())
                    <div>
                        <p class="text-[11px] text-gray-400 font-semibold uppercase tracking-wider mb-2">Foto Saat Ini</p>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            @foreach($lokasi->fotos as $foto)
                                <div class="relative group rounded-xl overflow-hidden border border-gray-100">
                                    <img src="{{ asset('storage/' . $foto->file_foto) }}" alt="Foto" class="w-full h-24 object-cover">
                                    <label class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer">
                                        <input type="checkbox" name="hapus_foto[]" value="{{ $foto->id_foto }}" class="sr-only peer">
                                        <span class="text-white text-[10px] font-bold uppercase tracking-wider peer-checked:text-red-400">Hapus</span>
                                    </label>
                                </div>
                            @endforeach
                        </div>
                        <p class="text-[11px] text-gray-400 mt-1">Hover dan klik untuk menandai foto yang akan dihapus</p>
                    </div>
                @endif

                <div>
                    <label for="foto" class="block text-sm font-semibold text-gray-700 mb-1.5">Tambah Foto Baru</label>
