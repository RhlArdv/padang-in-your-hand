...
@section('content')
    <form method="POST" action="{{ route('admin.events.update', $event->id_event) }}" enctype="multipart/form-data" class="max-w-3xl">
        @csrf @method('PUT')
        <div class="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="lokasi_event" id="lokasi_event" value="{{ old('lokasi_event', $event->lokasi_event) }}" required
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">
                </div>
                <div>
                    ...
                    <select name="jenis_event" id="jenis_event" required
                        ...
                        <option value="festival" {{ old('jenis_event', $event->jenis_event) == 'festival' ? 'selected' : '' }}>Festival</option>
                        <option value="wisata" {{ old('jenis_event', $event->jenis_event) == 'wisata' ? 'selected' : '' }}>Wisata</option>
                        <option value="olahraga" {{ old('jenis_event', $event->jenis_event) == 'olahraga' ? 'selected' : '' }}>Olahraga</option>
                        <option value="budaya" {{ old('jenis_event', $event->jenis_event) == 'budaya' ? 'selected' : '' }}>Budaya</option>
                ...
                @if(old('jenis_event', $event->jenis_event) == 'lainnya')
                ...
                @endif
                    ...
                    <input type="text" name="jenis_event_lainnya" id="jenis_event_lainnya" value="{{ old('jenis_event_lainnya', $jenisEventLainnya ?? '') }}"
                        class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10"
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:border-navy-500 focus:ring-2 focus:ring-navy-500/10">{{ old('deskripsi', $event->deskripsi) }}</textarea>
            </div>
            ...
            <div>
                ...
                @if($event->banner)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $event->banner) }}" alt="Banner" class="h-24 rounded-lg object-cover">
                    </div>
                @endif
                ...
                <p class="text-[11px] text-gray-400 mt-1">Kosongkan jika tidak ingin mengganti banner</p>
            </div>
        </div>
