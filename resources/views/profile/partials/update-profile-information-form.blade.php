<section class="space-y-6">
    ...
    <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="bg-gray-50/50 border border-gray-100 rounded-2xl p-5 flex flex-col sm:flex-row items-center gap-5">
            <div class="relative w-24 h-24 rounded-2xl overflow-hidden bg-gradient-to-br from-navy-700 to-navy-900 flex items-center justify-center text-white text-3xl font-bold border-2 border-white shadow-md flex-shrink-0 group">
                @if($user->foto)
                    <img id="avatar-preview" src="{{ asset('storage/' . $user->foto) }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                @else
                    @php
                    ...
                    @endphp
                    <span id="avatar-initials" class="group-hover:scale-105 transition-transform duration-300">{{ $initials }}</span>
                    <img id="avatar-preview" class="w-full h-full object-cover hidden group-hover:scale-105 transition-transform duration-300">
                @endif
            ...
            <div class="flex-1 text-center sm:text-left">
                <label for="foto" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">{{ __('Foto Profil') }}</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label for="name" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">{{ __('Nama Lengkap') }}</label>
            ...
            <div>
                <label for="no_hp" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">{{ __('Nomor HP') }}</label>
        ...
        <div>
            <label for="email" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">{{ __('Alamat Email') }}</label>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div class="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                    <p class="text-xs text-amber-800 flex items-center gap-1.5">
                        ...
                        <button form="send-verification" class="underline font-semibold text-amber-900 hover:text-amber-950">
